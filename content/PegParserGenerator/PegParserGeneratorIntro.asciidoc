...
In the abover PEG for matching a Sum Series of Integers, we have:

* Three non-terminal symbols: 'Integer', 'Sum' and 'Expression'
* Two terminal symbols: ++[0-9]\+ ++  and  ++ + ++
* One use of *Sequence*: 'Integer' \'+' 'Expression'
...
\'+' ++ to specify a literal ++ + ++ character. PEGs also provide some
regular-expression-esque constructs too, like: ++[0-9]\+ ++ and the
use of ++ * ++, ++\+ ++ and ++ ? ++.  However, any strings in PEGs are
just that - strings. *Not* rugular expressions. Vimpeg shuns (at
...
can more idiomatically be represented in the Vim regex as: ++\d\\\+
++.
...
NOTE: This is a provider-side utility function.
[source,vim]
---------------------------------------------------------------------
function! SumList(s)
